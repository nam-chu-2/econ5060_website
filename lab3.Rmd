...
-> Open up your do-file editor. Type in the following code:
...
-> Run your code and go back to the result window.

In the bottom left corner of the CLI, you will see some text with a bunch of slashes (forwards or backwards depending on your computer). This is called your <strong><a href=cheatsheets.html target = "_blank"> working directory </a></strong>, essentially the default place where STATA will save files or import them from.  
...
-> In the CLI, type `cd your_path_name` (where your_path_name is your path's name).

 Now you’ve changed your officially changed your working directory. Anything that you save or import using the command-line will be from your working directory.  
...
-> In your do-file, type `cd path_name` after `clear`.
...
-> before your `clear` command, type the following code
